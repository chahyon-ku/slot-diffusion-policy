...
class Decoder(nn.Module):
    def __init__(self, hid_dim, resolution, out_dim=4):
        super().__init__()
        ...
        self.conv6 = nn.ConvTranspose2d(hid_dim, out_dim + 1, 3, stride=(1, 1), padding=1)
...
class SlotAttentionAutoEncoder(nn.Module):
    def __init__(self, resolution, num_slots, num_iterations, hid_dim):
        ...
        self.decoder_cnn = Decoder(self.hid_dim, self.resolution, out_dim=self.hid_dim)
        self.recon_cnn = nn.Sequential(
            nn.Conv2d(self.hid_dim, self.hid_dim, 3, padding=1),
            nn.ReLU(),
            nn.Conv2d(self.hid_dim, self.hid_dim, 3, padding=1),
            nn.ReLU(),
            nn.Conv2d(self.hid_dim, 3, 3, padding=1),
        )
    ...
    def forward(self, src, tgt=None):
        ...
        feats, masks = x.reshape(image.shape[0], -1, x.shape[1], x.shape[2], x.shape[3]).split([64,1], dim=-1)
        # `feats` has shape: [batch_size, num_slots, width, height, num_channels].
        ...
        recons = self.recon_cnn(feats_recon)
