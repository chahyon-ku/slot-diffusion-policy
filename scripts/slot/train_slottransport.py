...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', default='./models3/', type=str, help='where to save models' )
    ...
    for epoch in range(opt.num_epochs):
        ...
        for sample in tqdm(train_dataloader):
            ...
            src = sample[0]['views']['front_rgb'].to(device)
            tgt = sample[1]['views']['front_rgb'].to(device)
            recon_combined, recons, masks, slots = model(src, tgt)
            loss = criterion(recon_combined, tgt)
            ...
            if not i % 5000:
                torch.save({
                    ...
                    os.path.join(opt.model_dir, 'model{}.ckpt'.format(i)))
